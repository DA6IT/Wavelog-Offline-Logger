...
`app.py` contains the Tkinter presentation layer; `logger_core.py` contains profiles, ADIF, metadata, Wavelog client and synchronization; dedicated modules handle CAT, callbook, external UDP logging, DX Cluster, notifications, backup, application updates, Hamlib updates and xOTA. `hamlib_update.py` verifies the official Windows archive and SHA-256 digest, tests the staged runtime and retains the previous runtime for rollback. User-visible translation is applied at the presentation boundary by `ui_preferences.py`; data values and ADIF remain language-neutral.
