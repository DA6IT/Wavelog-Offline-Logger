# Screenshot gallery 0.18.2

[Deutsch](../SCREENSHOTS.md) · **English**

All images are generated automatically with isolated demo data. No real ADI log, token, radio, network service or user profile is accessed.

- [QSO logging](../screenshots/en/qso-logging.png)
- [Fast Log](../screenshots/en/fast-log.png)
- [Contest logging](../screenshots/en/contest-logging.png)
- [xOTA](../screenshots/en/xota.png)
- [Logbook and sync](../screenshots/en/logbook-sync.png)
- [Statistics](../screenshots/en/statistics.png)
- [CAT Setup](../screenshots/en/cat-setup.png)
- [DX Cluster](../screenshots/en/dx-cluster.png)
- [UDP Logging](../screenshots/en/udp-logging.png)
- [Settings — General](../screenshots/en/settings-general.png)
- [Settings — Station & Wavelog](../screenshots/en/settings-wavelog.png)
- [Settings — Callbook](../screenshots/en/settings-callbook.png)
- [Settings — Data & connections](../screenshots/en/settings-data-connections.png)
- [Automatic sync running](../screenshots/en/sync-progress-running.png)
- [Automatic sync complete](../screenshots/en/sync-progress-complete.png)
